...
describe('Rover tests', () => {
  ...
  it('should throw InvalidDirectionError if the rover innitial direction is invalid', () => {
    ...
    expect(() => new Rover(plateau, [1, 1], 'A')).toThrow(new InvalidDirectionError('The direction A is not valid}'));
  });
  ...
  it('should move the rover correctly', () => {
    ...
    const rover1 = new Rover(plateau, [1, 1], 'N');
    rover1.move();
    expect(rover1.getPosition()).toBe('1 2 N');

    const rover2 = new Rover(plateau, [1, 1], 'E');
    rover2.move();
    expect(rover2.getPosition()).toBe('2 1 E');

    const rover3 = new Rover(plateau, [1, 1], 'S');
    rover3.move();
    expect(rover3.getPosition()).toBe('1 0 S');

    const rover4 = new Rover(plateau, [1, 1], 'W');
    rover4.move();
  ...
  it('should throw OutOfBoundsError if the rover goes out of bounds', () => {
    ...
    try {
      const rover = new Rover(plateau, [1, 1], 'N');
